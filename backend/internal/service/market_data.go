...
// QuoteService provides current stock price quotes, used for portfolio
// valuation and risk analysis — a live snapshot rather than a historical
// series, so it's kept as a separate interface from MarketDataService.
...
func NewCachedQuoteData(inner QuoteService, ttl time.Duration) *CachedQuoteData {
	return &CachedQuoteData{
		inner: inner,
		ttl:   ttl,
		cache: make(map[string]quoteCacheEntry),
	}
}

func (c *CachedQuoteData) GetQuote(ctx context.Context, symbol string) (*models.StockQuote, error) {
	c.mu.RLock()
	...
	c.mu.Lock()
	c.cache[symbol] = quoteCacheEntry{quote: quote, fetchedAt: time.Now()}
	c.mu.Unlock()

	return quote, nil
}

func (c *CachedQuoteData) GetBatchQuotes(ctx context.Context, symbols []string) (map[string]*models.StockQuote, error) {
	...
	for sym, quote := range fetched {
		c.cache[sym] = quoteCacheEntry{quote: quote, fetchedAt: time.Now()}
		results[sym] = quote
	}
	c.mu.Unlock()
